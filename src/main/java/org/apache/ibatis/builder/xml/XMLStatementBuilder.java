...
//    调用LanguageDriver.createSqlSource()方法创建SqlSource对象，前面解析<selectKey>节点时
//    介绍过该方法的具体实现，
...
//    这里会检测sql节点中是否配置了selectKey,SQL节点的useGeneratedKeys属性值，
//    mybatis-config.xml中全局的useGenerateKeys配置，以及是否为insert语句，决定使用的
//    keyGenerator接口实现，后面详细介绍keygenerator接口及其实现，
...
//    通过MapperBulderAssistant创建MappedStatement对象，并添加到Configuation.mappedStatements集合中保存，该集合为StrictMap<MappedStatement>类型
    builderAssistant.addMappedStatement(id, sqlSource, statementType, sqlCommandType,
        fetchSize, timeout, parameterMap, parameterTypeClass, resultMap, resultTypeClass,
        resultSetTypeEnum, flushCache, useCache, resultOrdered,
